...
const seedProfiles = require('./data/profiles.json');
...
mongoose.connect(MONGODB_URI).then(async () => {
  ...
  if (count === 0) await Profile.insertMany(seedProfiles);
...
app.post('/register', async (req, res) => {
  try {
    ...
    const user = await User.create({ email, password: hash, name, age, city, gender, lookingFor, hobbies, likedProfiles: [], matches: [] });
    req.session.userId = user._id.toString();
    res.redirect('/');
  } catch (e) {
...
app.get('/', auth, async (req, res) => {
  ...
  const query = { _id: { $nin: [...(user.likedProfiles || []), ...(user.matches || [])] } };
...
app.post('/like/:id', auth, async (req, res) => {
  ...
  const update = isMatch ? { $addToSet: { matches: req.params.id, likedProfiles: req.params.id } } : { $addToSet: { likedProfiles: req.params.id } };
...
app.post('/skip/:id', auth, async (req, res) => {
  await User.findByIdAndUpdate(req.session.userId, { $addToSet: { likedProfiles: req.params.id } });
  res.redirect('/');
});
